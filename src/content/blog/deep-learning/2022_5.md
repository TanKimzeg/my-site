---
title: "Lecture 5: Sequence to sequence"
...
这边课程提供的示例代码中的数据集是有问题的，链接资源404。我这边在Github上找到了其他仓库提供的数据集。在代码的 `Dataset Download` 章节的urls改为：

```python
urls = (
    "https://github.com/brantjao/ML2022-HW5Dataset/releases/download/v1.0.2/ted2020.tgz",
    "https://github.com/brantjao/ML2022-HW5Dataset/releases/download/v1.0.2/test.tgz",
)
```

同理 `ted_zh_corpus.deduped.gz` 的链接也改一下。这是目前还有效的链接。
...
反正意思就是output中出现target中的字越多效果越好。详见
[Metric评价指标-机器翻译指标之BLEU - 世界是我改变的的文章 - 知乎](https://zhuanlan.zhihu.com/p/350596071)。
...
这次不是直接运行示例代码那么简单了，[fairseq](https://github.com/facebookresearch/fairseq)的维护工作被抛弃了，自从2022年以来没有发布新版本，ISSUE和PR都爆了。我运行示例代码遇到了依赖冲突的问题，似乎需要跟 `python:3.8` 绑定。Colab保留的都是最新环境，解决起来非常麻烦，最好本地重新搞一个虚拟环境运行。
...
本次HW需要在HTU的JudgeBoi上提交结果，我们校外学生是无法使用的。通过Strong Baseline/Boss Baseline需要付出的时间成本也非常大。助教基本上也对超参数给予了很多提示。同时我缺乏本地硬件条件，在Kaggle/Colba上实验环境配置又比较麻烦。本次HW涉及的知识我已进行了学习。基于以上几点考量，这次的HW我就不亲自运行了。

此外，本次HW还引入了[wandb](https://docs.wandb.ai/models)对深度学习实验过程进行监控与可视化。它与HW01用到的tensorboard，是我以后写深度学习代码应该多采用的强大工具，以后有需要去学习一下。
