...
description: Hello-Agents第二部分 构建你的大语言模型智能体.包含四个章节:智能体经典范式构建、基于低代码平台的智能体搭建、框架开发实践、构建你的Agent框架.
pubDate: 2025 11 08 
categories: 
  - tech
tags:
...
本章我们将通过[Hello Agents源码](https://github.com/jjyaoao/HelloAgents)学习如何构建Agent框架.

HelloAgents框架的简洁,我是很喜欢的.但它使用的外部工具链我不是很喜欢,比如嵌入模型,向量数据库,图数据库稍显冗余,日志与打印混用,项目依赖管理落后等.我想自定义部署,采用更加轻量级的方案.否则得考虑用docker打包了.我想用HelloAgents框架写一个智能体,加入到项目展示里(见第十六章毕业设计),也想用自己喜欢的工具链调整HelloAgents,还想用现有成熟框架实现一遍,追求真正的通用和工程化.

RAG模块,我也想拓展之前基于langchain编写的接口.总之,HelloAgents框架简洁便于学习,但不够用.我觉得这一章应该放在上一章前面,这样,针对五花八门的现成框架,能够更好发现它们设计上不可或缺的共同点,能够根据自己的需要更有针对性的选择现有框架.上一章看完还是不太清楚Agent框架为什么这样设计,也不懂它们的区别.在这一章,我们对Agent框架有了更深的理解.

当天看到早上通勤的时候看到一篇[知乎回答](https://www.zhihu.com/question/1908469335023944724/answer/1935742258998649984),对Agent调用工具进行探讨,提到了输出格式不采用json而采用yaml,评论区甚至还有说用xml的.LLM调用工具的工作流依赖于输出格式,这一点十分脆弱.针对输出,Copilot似乎会听取语言服务器的检查结果.而有的Agent甚至调用另一个LLM对错误结果进行修复.正确调用工具的能力,是Agent的核心能力,并最终影响Agent的使用体验.

Agent的组件非常多,是非常庞大的工程.我个人的感受是工作流的设计模式虽然简单,但上限低;而事件驱动的设计模式(EDP)拥有更高上限.上一章讲的现有框架,有的是工作流,有的偏向EDP,在选择的时候应该体会到这种区别并纳入考量.
